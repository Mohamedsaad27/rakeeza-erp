...
--  ───────────────────────────────────────────────────────────
--  AUTH
--    - OAuth2 tables removed → replaced by Tymon JWT (stateless)
--    - users.remember_token removed (not needed for JWT)
--
--  IDs
--    - All surrogate PKs changed from BIGINT UNSIGNED AUTO_INCREMENT
--      to CHAR(36) UUID (Laravel Str::uuid() / Str::orderedUuid())
--    - All FK columns updated to CHAR(36) accordingly
--
--  SOFT DELETES
--    - deleted_at TIMESTAMP added to every table that lacked it
--
--  UNIQUE KEY TENANT SCOPING (bug fixes from v2 review)
--    - users: username + email unique keys now scoped to tenant_id
--    - contacts: contact_code unique key now scoped to tenant_id
--
--  TRANSACTIONS SPLIT
--    - Single `transactions` table replaced by three purpose-built tables:
--        sales_transactions      (sell, sell_return)
--        purchase_transactions   (purchase, purchase_return)
--        inventory_transactions  (transfer, opening_stock, spoiled_stock, adjustment)
--    - Corresponding line tables updated to reference the correct parent
--    - transfer_lines / spoiled_lines moved under inventory_transactions
--
--  CONTACTS
--    - activity_types given tenant_id (per-tenant classification)
--
--  PAYMENTS
--    - Added clear COMMENT on payment_transactions relationship to payments
--
--  PRODUCTS
--    - serial_numbers table added (was flagged but missing)
--    - batch_numbers table added (expiry tracking)
--
--  NAMING
--    - All constraint names use consistent snake_case prefixes
-- ============================================================
...
CREATE TABLE `subscription_plans` (
  `id`            CHAR(36)        NOT NULL,
  `name`          VARCHAR(100)    NOT NULL COMMENT 'Starter | Pro | Enterprise',
  `stripe_plan`   VARCHAR(255)    DEFAULT NULL COMMENT 'Stripe price ID for Cashier',
  `price`         DECIMAL(10,2)   NOT NULL DEFAULT 0.00,
  `billing_cycle` ENUM('monthly','yearly') NOT NULL DEFAULT 'monthly',
  `max_branches`  INT             NOT NULL DEFAULT 1,
  `max_users`     INT             NOT NULL DEFAULT 5,
  `features`      JSON            DEFAULT NULL COMMENT 'feature-flag JSON per plan',
  ...
  COMMENT='SaaS subscription plans.';
...
CREATE TABLE `users` (
  ...
  `super`         TINYINT(1)      NOT NULL DEFAULT 0 COMMENT '1 = platform super-admin',
  `is_active`     TINYINT(1)      NOT NULL DEFAULT 1,
...
CREATE TABLE `roles` (
  ...
  `deleted_at`   TIMESTAMP       NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `roles_name_tenant_guard_unique` (`tenant_id`, `name`, `guard_name`),
...
CREATE TABLE `permissions` (
  `id`           CHAR(36)        NOT NULL,
  `name`         VARCHAR(255)    NOT NULL COMMENT 'e.g. sales.invoices.create',
  `guard_name`   VARCHAR(255)    NOT NULL DEFAULT 'api',
  ...
  COMMENT='Global permission definitions. Seeded once, shared across all tenants.';
...
CREATE TABLE `serial_numbers` (
  ...
  CONSTRAINT `sn_product_id_foreign`
    FOREIGN KEY (`product_id`)  REFERENCES `products`   (`id`) ON DELETE CASCADE,
  CONSTRAINT `sn_warehouse_id_foreign`
...
CREATE TABLE `batch_numbers` (
  ...
  CONSTRAINT `bn_product_id_foreign`
    FOREIGN KEY (`product_id`)   REFERENCES `products`   (`id`) ON DELETE CASCADE,
  CONSTRAINT `bn_warehouse_id_foreign`
    FOREIGN KEY (`warehouse_id`) REFERENCES `warehouses` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE `media` (
  ...
  KEY `media_model_type_model_id_idx` (`model_type`, `model_id`),
  KEY `media_order_column_idx`        (`order_column`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Spatie Medialibrary.';
...
CREATE TABLE `account_types` (
  `id`             CHAR(36)        NOT NULL,
  `name`           VARCHAR(100)    NOT NULL COMMENT 'Asset | Liability | Equity | Revenue | Expense',
...
CREATE TABLE `taxes` (
  `id`         CHAR(36)        NOT NULL,
  `tenant_id`  CHAR(36)        NOT NULL,
  `name`       VARCHAR(100)    NOT NULL COMMENT 'e.g. VAT 14%, Withholding 5%',
...
CREATE TABLE `leave_types` (
  `id`           CHAR(36)        NOT NULL,
  `tenant_id`    CHAR(36)        NOT NULL,
  `name`         VARCHAR(100)    NOT NULL COMMENT 'Annual | Sick | Emergency',
...
CREATE TABLE `salary_components` (
  `id`          CHAR(36)        NOT NULL,
  `tenant_id`   CHAR(36)        NOT NULL,
  `name`        VARCHAR(255)    NOT NULL COMMENT 'Housing Allowance | Social Insurance',
...
CREATE TABLE `activity_log` (
  ...
  COMMENT='Spatie Activitylog — audit trail for all model changes.';
...
-- ============================================================
--  END OF SCHEMA  — RAKEEZA ERP v3.0
-- ============================================================
--
--  COMPLETE CHANGE SUMMARY v2 → v3
--  ─────────────────────────────────────────────────────────
--
--  AUTH
--    oauth_clients           REMOVED  → Tymon JWT (stateless, no DB tables needed)
--    oauth_access_tokens     REMOVED
--    oauth_refresh_tokens    REMOVED
--    users.remember_token    REMOVED  → JWT is stateless
--
--  IDs
--    All BIGINT UNSIGNED AUTO_INCREMENT PKs → CHAR(36) UUID
--    All BIGINT UNSIGNED FK columns         → CHAR(36)
--    Laravel model: use HasUuids trait or boot() with Str::orderedUuid()
--
--  SOFT DELETES (deleted_at added to all tables)
--    subscription_plans, governorates, cities, password_resets,
--    user_branches, permissions, role_has_permissions,
--    model_has_roles, model_has_permissions, activity_types,
--    product_price_histories, media, account_types, journal_entry_lines,
--    taxes, stock_levels, stock_adjustment_lines, purchase_order_lines,
--    quotation_lines, order_items, employee_salary_components,
--    transaction_update_histories, kpi_snapshots, failed_jobs,
--    migrations — all now carry deleted_at.
--
--  BUG FIXES FROM v2 REVIEW
--    users.username unique          → scoped to (tenant_id, username)
--    users.email unique             → scoped to (tenant_id, email)
--    contacts.contact_code unique   → scoped to (tenant_id, contact_code)
--    activity_types                 → now per-tenant (added tenant_id FK)
--    spoiled_lines constraint clash → resolved by merging into inventory_transaction_lines
--    orders table                   → fully fleshed out (branch, warehouse, created_by, ref_no, dates)
--
--  TRANSACTIONS SPLIT
--    transactions            REMOVED
--    transaction_sell_lines  REMOVED  → sales_transaction_lines
--    transaction_purchase_lines REMOVED → purchase_transaction_lines
--    transfer_lines          REMOVED  → inventory_transaction_lines
--    spoiled_lines           REMOVED  → inventory_transaction_lines
--
--    NEW: sales_transactions         (sell, sell_return)
--    NEW: sales_transaction_lines
--    NEW: purchase_transactions      (purchase, purchase_return)
--    NEW: purchase_transaction_lines
--    NEW: inventory_transactions     (transfer, opening_stock, spoiled_stock, adjustment)
--    NEW: inventory_transaction_lines
--
--  NEW TABLES
--    serial_numbers    — tracks individual serialized items (is_serialized products)
--    batch_numbers     — tracks batch/lot with expiry dates (has_expiry products)
--
--  PAYMENT CLARITY
--    payment_transactions.transaction_type ENUM added (sales | purchase)
--    payment_transactions.transaction_id   is now a polymorphic UUID reference
--    COMMENT added to distinguish payments vs payment_transactions
--
--  journal_entries.reference_type updated to include:
--    sales_transaction | purchase_transaction | inventory_transaction | payment | expense
-- ============================================================
